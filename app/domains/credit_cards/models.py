...
class CreditCard(Base):
    ...
    available_credit = Column(DECIMAL(15, 2), nullable=True)  # credit_limit - current_balance
    ...
    __table_args__ = (
        ...
        UniqueConstraint("name", "broker_id", "account_id", name="uix_name_broker_account"),
        ...
        CheckConstraint('credit_limit IS NULL OR (credit_limit >= 0 AND credit_limit <= 999999999999999.99)', name='credit_limit_precision'),
        CheckConstraint('current_balance >= -999999999999999.99 AND current_balance <= 999999999999999.99', name='current_balance_precision'),
        CheckConstraint('available_credit IS NULL OR (available_credit >= -999999999999999.99 AND available_credit <= 999999999999999.99)', name='available_credit_precision'),
        
        # Business logic constraints
        CheckConstraint('credit_limit IS NULL OR credit_limit >= 0', name='positive_credit_limit'),
    )
