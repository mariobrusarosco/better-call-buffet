...
def upgrade() -> None:
    ...
    op.alter_column('credit_cards', 'due_date',
               ...
               nullable=True)
    op.alter_column('credit_cards', 'balance',
               existing_type=sa.DOUBLE_PRECISION(precision=53),
               nullable=True)
    op.create_index(op.f('ix_credit_cards_balance'), 'credit_cards', ['balance'], unique=False)
    op.create_index(op.f('ix_credit_cards_brand'), 'credit_cards', ['brand'], unique=False)
    op.create_index(op.f('ix_credit_cards_due_date'), 'credit_cards', ['due_date'], unique=False)
    op.create_index(op.f('ix_credit_cards_is_active'), 'credit_cards', ['is_active'], unique=False)
    op.create_index(op.f('ix_credit_cards_is_deleted'), 'credit_cards', ['is_deleted'], unique=False)
...
def downgrade() -> None:
    ...
    op.drop_constraint(None, 'accounts', type_='foreignkey')
    op.create_foreign_key('accounts_broker_id_fkey', 'accounts', 'brokers', ['broker_id'], ['id'], ondelete='CASCADE')
